...
def init():
    ...
    sensors = [('n/a',0x53, 'spec1', 'TOX',1,1,0,0,'raw'),
               ('n/a',0x53, 'spec2', 'TOR',1,1,0,0,'raw'),
                ('n/a',0x55, 'spec1', 'SO2',1,1,0,0,'raw'),
                ('n/a',0x55, 'spec2', 'H2S',1,1,0,0,'raw'),
                ('n/a',0x56, 'spec1', 'O3-',1,1,0,0,'raw'),
                ('n/a',0x56, 'spec2', 'NO2',1,1,0,0,'raw'),
                ('n/a',0x57, 'spec1', 'TOX',1,1,0,0,'raw'),
                ('n/a',0x57, 'spec2', 'CO-',1,1,0,0,'raw')
              ]
    c.executemany('INSERT INTO eeprom VALUES (?,?,?,?,?,?,?,?,?)', sensors)       
    c.execute('''DROP TABLE IF EXISTS calibrate''')
    c.execute('''CREATE TABLE calibrate (id INTEGER PRIMARY KEY AUTOINCREMENT,'AAunit' TEXT DEFAULT 'ppm','ABunit' TEXT DEFAULT 'ppm','BAunit' TEXT DEFAULT 'ppm','BBunit' TEXT DEFAULT 'ppm','CAunit' TEXT DEFAULT 'ppm','CBunit' TEXT DEFAULT 'ppm','DAunit' TEXT DEFAULT 'ppm','DBunit' TEXT DEFAULT 'ppm','AAaddr' TEXT DEFAULT '87','AAsel' TEXT DEFAULT 'CO-', 'AA_sen' TEXT DEFAULT '10.02', 'AA_base' TEXT DEFAULT '16.76', 'AA_zero' TEXT DEFAULT '0', 'AA_span' TEXT DEFAULT '0.6', 'ABaddr' TEXT DEFAULT '87','ABsel' TEXT DEFAULT 'NO2', 'AB_sen' TEXT DEFAULT '-39.91', 'AB_base' TEXT DEFAULT '-0.18', 'AB_zero' TEXT DEFAULT '0', 'AB_span' TEXT DEFAULT '0', 'BAaddr' TEXT DEFAULT '86','BAsel' TEXT DEFAULT 'O3-', 'BA_sen' TEXT DEFAULT '-14.05', 'BA_base' TEXT DEFAULT '3.90', 'BA_zero' TEXT DEFAULT '0', 'BA_span' TEXT DEFAULT '0', 'BBaddr' TEXT DEFAULT '86','BBsel' TEXT DEFAULT 'SO2', 'BB_sen' TEXT DEFAULT '34.04', 'BB_base' TEXT DEFAULT '44.01', 'BB_zero' TEXT DEFAULT '0', 'BB_span' TEXT DEFAULT '1.2', 'CAaddr' TEXT DEFAULT '85','CAsel' TEXT DEFAULT 'H2S', 'CA_sen' TEXT DEFAULT '189.49', 'CA_base' TEXT DEFAULT '87.24', 'CA_zero' TEXT DEFAULT '0', 'CA_span' TEXT DEFAULT '0.3', 'CBaddr' TEXT DEFAULT '85', 'CBsel' TEXT DEFAULT 'TOR', 'CB_sen' TEXT DEFAULT '-49.51', 'CB_base' TEXT DEFAULT '3.33', 'CB_zero' TEXT DEFAULT '0', 'CB_span' TEXT DEFAULT '0', 'DAaddr' TEXT DEFAULT '83', 'DAsel' TEXT DEFAULT 'TOX', 'DA_sen' TEXT DEFAULT '15.37', 'DA_base' TEXT DEFAULT '161.41', 'DA_zero' TEXT DEFAULT '0', 'DA_span' TEXT DEFAULT '0', 'DBaddr' TEXT DEFAULT '83',  'DBsel' TEXT DEFAULT 'TOX', 'DB_sen' TEXT DEFAULT '15.37', 'DB_base' TEXT DEFAULT '161.41', 'DB_zero' TEXT DEFAULT '0', 'DB_span' TEXT DEFAULT '0')''')  
...
def ui2eepromTransfer():
    ...
    spec1 = {}   
    spec2 = {}
    query = str('SELECT AAaddr, AAsel, AA_sen, AA_base, AA_zero, AA_span, AAunit, ABaddr,ABsel, AB_sen, AB_base, AB_zero, AB_span, ABunit, BAaddr, BAsel,  BA_sen, BA_base, BA_zero, BA_span, BAunit, BBaddr, BBsel, BB_sen, BB_base, BB_zero, BB_span, BBunit, CAaddr, CAsel, CA_sen, CA_base, CA_zero, CA_span, CAunit, CBaddr, CBsel, CB_sen, CB_base, CB_zero, CB_span, CBunit, DAaddr, DAsel, DA_sen, DA_base, DA_zero, DA_span, DAunit, DBaddr, DBsel, DB_sen, DB_base, DB_zero, DB_span, DBunit FROM calibrate ORDER BY id DESC')
    c.execute(query)    
    params = c.fetchall()
    parameters = ",".join(map(str,params[0][0:])).split(',')
    print parameters
    while len(parameters) !=0:
        spec1[parameters.pop()] = [parameters.pop() for i in range(6)]
        spec2[parameters.pop()] = [parameters.pop() for i in range(6)]

    for key in spec1.iterkeys():
        query = str('UPDATE eeprom SET unit="%s",sensitivity="%s", baseline="%s", offset="%s", span="%s", type="%s" WHERE addr=%s and socket="spec1"') % ((str(spec1[key][0]), str(spec1[key][1]), str(spec1[key][2]), str(spec1[key][3]), str(spec1[key][4]), str(spec1[key][5]), str(key)))       
        print query        
        c.execute(query)
        #conn.commit()  
    for key in spec2.iterkeys():
        query = str('UPDATE eeprom SET unit="%s",sensitivity="%s", baseline="%s", offset="%s", span="%s", type="%s" WHERE addr=%s and socket="spec2"') % ((str(spec2[key][0]), str(spec2[key][1]), str(spec2[key][2]), str(spec2[key][3]), str(spec2[key][4]), str(spec2[key][5]), str(key)))        
        print query
        c.execute(query)
    conn.commit()
    conn.close()
